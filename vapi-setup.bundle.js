...
Reason: ${e4}`);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof import_web.default !== "undefined" && VAPI_PUBLIC_KEY) {
      vapiInstance = new import_web.default(VAPI_PUBLIC_KEY);
      ...
      vapiInstance.on("call-start", () => {
        ...
        vapiMicBtn.innerHTML = '<span style="font-size: 32px; color: white;">\u{1F6D1}</span>';
        if (vapiTranscript) {
      ...
      vapiInstance.on("call-end", () => {
        ...
        vapiMicBtn.innerHTML = '<span style="font-size: 32px; color: white;">\u{1F3A4}</span>';
      });
      vapiInstance.on("message", (message) => {
        ...
        if (message.type === "transcript" && message.transcriptType === "final") {
          ...
          if (role === "assistant" && vapiStatCard) {
            ...
            if (vapiStatHead) vapiStatHead.textContent = text.length > 120 ? text.substring(0, 117) + "\u2026" : text;
          }
        }
      });
